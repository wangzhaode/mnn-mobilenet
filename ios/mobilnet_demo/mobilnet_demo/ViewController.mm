...
//  mobilnet_demo
//
//  Created by wangzhaode on 2023/3/6.
//
...
- (instancetype)init {
    self = [super init];
    if (self) {
        NSString *labels  = [[NSBundle mainBundle] pathForResource:@"names" ofType:@"txt"];
        ...
        self.defaultImage = [UIImage imageNamed:@"cat.jpg"];

        NSString *model = [[NSBundle mainBundle] pathForResource:@"mobilenet_v1" ofType:@"mnn"];
...
- (NSString *)inferImage:(UIImage *)image cycles:(NSInteger)cycles {
    ...
    pretreat->convert(rgba, w, h, 0, givenTensor.get());
...
- (NSString *)inferBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    auto input = _net->getSessionInput(_session, nullptr);
    std::shared_ptr<MNN::Tensor> givenTensor(MNN::Tensor::createHostTensorFromDevice(input, false));
    pretreat->convert(bgra, w, h, 0, givenTensor.get());
    input->copyFromHostTensor(givenTensor.get());
...
@property (strong, nonatomic) Model *mobileNet;
...
- (void)awakeFromNib {
    ...
    self.threadCount    = 4;
    self.mobileNet    = [MobileNet new];
    self.currentModel   = self.mobileNet;
...
- (IBAction)toggleMode {
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"CoreML"
                                              style:UIAlertActionStyleDefault
                                            handler:^(UIAlertAction *action) {
                                                __strong typeof(weakify) self = weakify;
                                                self.forwardItem.title        = action.title;
                                                self.forwardType              = MNN_FORWARD_NN;
                                                [self refresh];
                                            }]];
